...
class HardwareModel:
	...
	# 	'''
	# 	cfg is a string which tells us which hardware configuration to use
	# 	this should already be specified in the config file.
	# 	'''
	...
	def set_technology_parameters(self):
		self.area = area[self.transistor_size]
		self.latency = latency[self.transistor_size]
		self.latency_scale = latency_scale
		self.dynamic_power = dynamic_power[self.transistor_size]
		self.leakage_power = leakage_power[self.transistor_size]

		if self.transistor_size == 3:
			self.area["Regs"] = mem_area_3[self.cache_size][self.mem_layers][self.pitch]
		else:
			self.area["Regs"] = mem_area_7_5[self.cache_size][self.mem_layers][self.pitch]
		
		self.latency["Regs"] = mem_latency[self.cache_size][self.mem_layers][self.pitch]
		self.dynamic_power["Regs"] = mem_dynamic_power[self.cache_size][self.mem_layers][self.pitch]
		self.leakage_power["Regs"] = mem_leakage_power[self.cache_size][self.mem_layers][self.pitch]
