#include <stdint.h>

/* ------------------------ Macros and S-box ------------------------ */

static inline uint8_t F_u8(uint8_t x) {
    uint8_t hi   = (uint8_t)(x >> 7);
    uint8_t xt   = (uint8_t)(x << 1);
    uint8_t mask = (uint8_t)(hi * 0x1b);
    return (uint8_t)(xt ^ mask);
}

static inline uint8_t FD_u8(uint8_t x) {
    uint8_t lsb  = (uint8_t)(x & 1u);
    uint8_t shr  = (uint8_t)(x >> 1);
    uint8_t mask = (uint8_t)(lsb ? 0x8d : 0x00);
    return (uint8_t)(shr ^ mask);
}




static inline uint8_t rj_sbox_fn(uint8_t x) {
    static uint8_t sbox[256] = {
        
    };
    return sbox[x];
}

/* ------------------------ Small helpers ----------------------------------- */

static inline uint8_t rj_xtime(uint8_t x) {
    return (uint8_t)(((x & 0x80u) != 0u) ? ((x << 1) ^ 0x1bu) : (x << 1));
}

static inline uint8_t xt(uint8_t x) {
    return rj_xtime(x);
}

/* ------------------------ Round operations (16-byte state) ---------------- */

void aes_subBytes(uint8_t buf[16]) {
    int i;
    for (i = 0; i < 16; ++i) {
        buf[i] = rj_sbox_fn(buf[i]);
    }
}

void aes_addRoundKey(uint8_t buf[16], uint8_t key[16]) {
    int i;
    for (i = 0; i < 16; ++i) {
        buf[i] = (uint8_t)(buf[i] ^ key[i]);
    }
}

/* key32 and round32 are 32-byte working key buffers */
void aes_addRoundKey_cpy(uint8_t buf[16], uint8_t key32[32], uint8_t round32[32]) {
    int i;
    /* copy full 32-byte key into round32, and XOR first 16 bytes into buf */
    for (i = 0; i < 16; ++i) {
        uint8_t v0 = key32[i];
        uint8_t v1 = key32[16 + i];
        buf[i]       = (uint8_t)(buf[i] ^ v0);
        round32[i]   = v0;
        round32[16 + i] = v1;
    }
}

void aes_shiftRows(uint8_t buf[16]) {
    int r, c;

    /* row 0 unchanged (r = 0) */

    /* rows 1..3: left rotate by r bytes within each row */
    for (r = 1; r < 4; ++r) {
        int shift = r;          /* row 1 → shift 1, row 2 → 2, row 3 → 3 */
        uint8_t tmp[4];

        /* read row r into tmp with rotation */
        for (c = 0; c < 4; ++c) {
            int src_col = (c + shift) & 3;          /* (c + shift) % 4 */
            int src_idx = 4 * r + src_col;
            tmp[c] = buf[src_idx];
        }

        /* write rotated row back */
        for (c = 0; c < 4; ++c) {
            int dst_idx = 4 * r + c;
            buf[dst_idx] = tmp[c];
        }
    }
}

void aes_mixColumns(uint8_t buf[16]) {
    int i;
    uint8_t a, b, c, d, e;

    for (i = 0; i < 16; i += 4) {
        a = buf[i];
        b = buf[i + 1];
        c = buf[i + 2];
        d = buf[i + 3];

        e = (uint8_t)(a ^ b ^ c ^ d);

        buf[i]     = (uint8_t)(buf[i]     ^ e ^ xt((uint8_t)(a ^ b)));
        buf[i + 1] = (uint8_t)(buf[i + 1] ^ e ^ xt((uint8_t)(b ^ c)));
        buf[i + 2] = (uint8_t)(buf[i + 2] ^ e ^ xt((uint8_t)(c ^ d)));
        buf[i + 3] = (uint8_t)(buf[i + 3] ^ e ^ xt((uint8_t)(d ^ a)));
    }
}

/* 32-byte key schedule update */
void aes_expandEncKey(uint8_t k[32], uint8_t *rc) {
    int i;

    k[0] = (uint8_t)(k[0] ^ rj_sbox_fn(k[29]) ^ *rc);
    k[1] = (uint8_t)(k[1] ^ rj_sbox_fn(k[30]));
    k[2] = (uint8_t)(k[2] ^ rj_sbox_fn(k[31]));
    k[3] = (uint8_t)(k[3] ^ rj_sbox_fn(k[28]));

    *rc = F_u8(*rc);

    for (i = 4; i < 16; ++i) {
        k[i] = (uint8_t)(k[i] ^ k[i - 4]);
    }

    k[16] = (uint8_t)(k[16] ^ rj_sbox_fn(k[12]));
    k[17] = (uint8_t)(k[17] ^ rj_sbox_fn(k[13]));
    k[18] = (uint8_t)(k[18] ^ rj_sbox_fn(k[14]));
    k[19] = (uint8_t)(k[19] ^ rj_sbox_fn(k[15]));

    for (i = 20; i < 32; ++i) {
        k[i] = (uint8_t)(k[i] ^ k[i - 4]);
    }
}

void aes_addRoundKey32(uint8_t buf[16], uint8_t key32[32], int base) {
    int i;
    for (i = 0; i < 16; ++i) {
        buf[i] = (uint8_t)(buf[i] ^ key32[base + i]);
    }
}

/* ------------------------ Top kernel: AES-256 encrypt --------------------- */
/* This should be the ScaleHLS top function (no struct argument).            */

void aes(uint8_t key_in[32], uint8_t buf[16]) {
    uint8_t enckey[32];
    uint8_t deckey[32];
    uint8_t roundkey[32];
    uint8_t rcon = 1;
    int i, j;

    /* initialize key buffers */
    for (i = 0; i < 32; ++i) {
        uint8_t v = key_in[i];
        enckey[i] = v;
        deckey[i] = v;
    }

    /* pre-expand decryption key 7 times (original code loop from 8 down) */
    for (i = 0; i < 7; ++i) {
        aes_expandEncKey(deckey, &rcon);
    }

    /* first round key addition + copy to roundkey */
    aes_addRoundKey_cpy(buf, enckey, roundkey);

    rcon = 1;

    /* 13 main rounds */
    for (j = 1; j < 14; ++j) {
        aes_subBytes(buf);
        aes_shiftRows(buf);
        aes_mixColumns(buf);

        if (j & 1) {
            /* use upper half of roundkey */
            aes_addRoundKey32(buf, roundkey, 16);
        } else {
            aes_expandEncKey(roundkey, &rcon);
            aes_addRoundKey32(buf, roundkey, 0);
        }
    }

    /* final round (no mixColumns) */
    aes_subBytes(buf);
    aes_shiftRows(buf);
    aes_expandEncKey(roundkey, &rcon);
    aes_addRoundKey32(buf, roundkey, 0);
}
