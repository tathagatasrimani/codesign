...
class CircuitModel:
    ...
    def update_circuit_values(self):
        ...
        self.circuit_values["latency"] = {
            key: float(self.symbolic_latency_wc[key]().subs(self.tech_model.base_params.tech_values)) for key in self.symbolic_latency_wc if key not in ["Buf", "MainMem", "OffChipIO"]
        }
        self.circuit_values["dynamic_energy"] = {
            key: float(self.symbolic_energy_active[key]().subs(self.tech_model.base_params.tech_values)) for key in self.symbolic_energy_active if key not in ["Buf", "MainMem", "OffChipIO"]
        }
        self.circuit_values["passive_power"] = {
            key: float(self.symbolic_power_passive[key]().subs(self.tech_model.base_params.tech_values)) for key in self.symbolic_power_passive if key not in ["Buf", "MainMem"]
        }
        self.circuit_values["area"] = {
            key: float(self.symbolic_area[key]().subs(self.tech_model.base_params.tech_values)) for key in self.symbolic_area
        }
