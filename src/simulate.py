...
class ConcreteSimulator(AbstractSimulator):
    ...
    def calculate_passive_energy(self, total_execution_time):
        passive_power = 0
        for node in self.hw.netlist:
            data = self.hw.netlist.nodes[node]
            if data["function"] == "Buf" or data["function"] == "MainMem":
                rsc_name = data["library"][data["library"].find("__")+1:]
                passive_power += self.hw.params.memories[rsc_name]["Standby leakage per bank(mW)"] * 1e6 # convert from mW to nW
            else:
                passive_power += self.hw.params.circuit_values["leakage_power"][data["function"]]
        ...
        for node in scheduled_dfg:
            ...
            if data["function"] == "Buf" or data["function"] == "MainMem":
                ...
                if data["module"].find("wport") != -1:
                    self.total_active_energy += self.hw.params.memories[rsc_name]["Dynamic write energy (nJ)"]
                else:
                    self.total_active_energy += self.hw.params.memories[rsc_name]["Dynamic read energy (nJ)"]
            else:
                energy = self.hw.params.circuit_values["dynamic_power"][data["function"]] * self.hw.params.circuit_values["latency"][data["function"]]*1e-9
                self.total_active_energy += energy
                logger.info(f"(active energy) {data['function']}: {energy}")
    
    def calculate_edp(self, scheduled_dfg):
