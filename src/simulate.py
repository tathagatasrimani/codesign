...
class HardwareSimulator:
    ...
    def get_hw_need(self, state, hw_spec):
        ...
        for op in state:
            ...
            if op.operation != "Regs":
                compute_element_id = (
                    hw_need.hw_allocated[op.operation]
                    % hw_spec.hw_allocated[op.operation]
                )
                # print(f"compute_element_id: {compute_element_id}; compute_element_to_node_id: {self.compute_element_to_node_id}")
                if (
                    len(self.compute_element_to_node_id[op.operation])
                    <= compute_element_id
                ):
                    # print(f"entered ")
    ...
    def cycle_sim(self, hw_inuse, hw, max_cycles):
        ...
        for i in range(max_cycles):
            ...
            for elem in hw_inuse:
                ...
                if elem == "Regs":
                    power *= hw.power_scale[
                        self.find_nearest_mem_to_scale(self.memory_needed)
                    ]
    ...
    def process_compute_element_neighbor(
        self, neighbor, graph, op_node, context, check_duplicate
    ):
        name = neighbor.value
        mem_size = 0
    ...
    def process_compute_element(self, op, graph, op_node, check_duplicate):
        ...
        for parent in op.parents:
            ...
            if parent.operation == "Regs":
                mem_in_use += self.process_compute_element_neighbor(
        ...
        while i < len(self.data_path):
            ...
            while idx < len(pattern_nodes):
                ...
                for operations in node_operation_map[cur_node]:
                    ...
                    op_count = 0
                    for op in operations:
                        if not op.operation or op.operation == "Regs":
                            continue
                        op_count += 1
