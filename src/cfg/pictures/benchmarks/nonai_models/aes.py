digraph "clustermain.c" {
	graph [label="main.c"]
	1 [label="Sbox = (99, 124, 119, 123, 242, 107, 111, 197, 48, 1, 103, 43, 254, 215, 
    171, 118, 202, 130, 201, 125, 250, 89, 71, 240, 173, 212, 162, 175, 156,
    164, 114, 192, 183, 253, 147, 38, 54, 63, 247, 204, 52, 165, 229, 241, 
    113, 216, 49, 21, 4, 199, 35, 195, 24, 150, 5, 154, 7, 18, 128, 226, 
    235, 39, 178, 117, 9, 131, 44, 26, 27, 110, 90, 160, 82, 59, 214, 179, 
    41, 227, 47, 132, 83, 209, 0, 237, 32, 252, 177, 91, 106, 203, 190, 57,
    74, 76, 88, 207, 208, 239, 170, 251, 67, 77, 51, 133, 69, 249, 2, 127, 
    80, 60, 159, 168, 81, 163, 64, 143, 146, 157, 56, 245, 188, 182, 218, 
    33, 16, 255, 243, 210, 205, 12, 19, 236, 95, 151, 68, 23, 196, 167, 126,
    61, 100, 93, 25, 115, 96, 129, 79, 220, 34, 42, 144, 136, 70, 238, 184,
    20, 222, 94, 11, 219, 224, 50, 58, 10, 73, 6, 36, 92, 194, 211, 172, 98,
    145, 149, 228, 121, 231, 200, 55, 109, 141, 213, 78, 169, 108, 86, 244,
    234, 101, 122, 174, 8, 186, 120, 37, 46, 28, 166, 180, 198, 232, 221, 
    116, 31, 75, 189, 139, 138, 112, 62, 181, 102, 72, 3, 246, 14, 97, 53, 
    87, 185, 134, 193, 29, 158, 225, 248, 152, 17, 105, 217, 142, 148, 155,
    30, 135, 233, 206, 85, 40, 223, 140, 161, 137, 13, 191, 230, 66, 104, 
    65, 153, 45, 15, 176, 84, 187, 22)
InvSbox = (82, 9, 106, 213, 48, 54, 165, 56, 191, 64, 163, 158, 129, 243, 
    215, 251, 124, 227, 57, 130, 155, 47, 255, 135, 52, 142, 67, 68, 196, 
    222, 233, 203, 84, 123, 148, 50, 166, 194, 35, 61, 238, 76, 149, 11, 66,
    250, 195, 78, 8, 46, 161, 102, 40, 217, 36, 178, 118, 91, 162, 73, 109,
    139, 209, 37, 114, 248, 246, 100, 134, 104, 152, 22, 212, 164, 92, 204,
    93, 101, 182, 146, 108, 112, 72, 80, 253, 237, 185, 218, 94, 21, 70, 87,
    167, 141, 157, 132, 144, 216, 171, 0, 140, 188, 211, 10, 247, 228, 88, 
    5, 184, 179, 69, 6, 208, 44, 30, 143, 202, 63, 15, 2, 193, 175, 189, 3,
    1, 19, 138, 107, 58, 145, 17, 65, 79, 103, 220, 234, 151, 242, 207, 206,
    240, 180, 230, 115, 150, 172, 116, 34, 231, 173, 53, 133, 226, 249, 55,
    232, 28, 117, 223, 110, 71, 241, 26, 113, 29, 41, 197, 137, 111, 183, 
    98, 14, 170, 24, 190, 27, 252, 86, 62, 75, 198, 210, 121, 32, 154, 219,
    192, 254, 120, 205, 90, 244, 31, 221, 168, 51, 136, 7, 199, 49, 177, 18,
    16, 89, 39, 128, 236, 95, 96, 81, 127, 169, 25, 181, 74, 13, 45, 229, 
    122, 159, 147, 201, 156, 239, 160, 224, 59, 77, 174, 42, 245, 176, 200,
    235, 187, 60, 131, 83, 153, 97, 23, 43, 4, 126, 186, 119, 214, 38, 225,
    105, 20, 99, 85, 33, 12, 125)
def xtime(a):...
Rcon = (0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54, 108, 216, 171, 77, 154, 47,
    94, 188, 99, 198, 151, 53, 106, 212, 179, 125, 250, 239, 197, 145, 57)
round_keys = []
def text2matrix(text):...
def matrix2text(matrix):...
def __init__(master_key):...
def change_key(master_key):...
def encrypt(plaintext):...
def decrypt(ciphertext):...
def __add_round_key(s, k):...
def __round_encrypt(state_matrix, key_matrix):...
def __round_decrypt(state_matrix, key_matrix):...
def __sub_bytes(s):...
def __inv_sub_bytes(s):...
def __shift_rows(s):...
def __inv_shift_rows(s):...
def __mix_single_column(a):...
def __mix_columns(s):...
def __inv_mix_columns(s):...
if __name__ == '__main__':
"]
	106 [label="import time
start = time.time()
aes = __init__(1212304810341341)
encrypt(1212304810341341)
end = time.time()
print(end - start)
"]
	"106_calls" [label="time.time
__init__
encrypt
time.time
print" shape=box]
	106 -> "106_calls" [label=calls style=dashed]
	1 -> 106 [label="__name__ == '__main__'"]
	subgraph clusterxtime {
		graph [label=xtime]
		3 [label="if a & 128:
"]
		4 [label="return (a << 1 ^ 27) & 255
"]
		3 -> 4 [label="a & 128"]
		6 [label="return a << 1
"]
		3 -> 6 [label="(not a & 128)"]
	}
	subgraph clustertext2matrix {
		graph [label=text2matrix]
		11 [label="matrix = []
"]
		12 [label="for i in range(16):
"]
		13 [label="byte = text >> 8 * (15 - i) & 255
if i % 4 == 0:
"]
		15 [label="matrix.append([byte])
"]
		"15_calls" [label="matrix.append" shape=box]
		15 -> "15_calls" [label=calls style=dashed]
		15 -> 12 [label=""]
		13 -> 15 [label="i % 4 == 0"]
		17 [label="matrix[int(i / 4)].append(byte)
"]
		"17_calls" [label="matrix.append" shape=box]
		17 -> "17_calls" [label=calls style=dashed]
		17 -> 12 [label=""]
		13 -> 17 [label="(i % 4 != 0)"]
		12 -> 13 [label="range(16)"]
		14 [label="return matrix
"]
		12 -> 14 [label=""]
		11 -> 12 [label=""]
	}
	subgraph clustermatrix2text {
		graph [label=matrix2text]
		21 [label="text = 0
"]
		22 [label="for i in range(4):
"]
		23 [label="for j in range(4):
"]
		25 [label="text |= matrix[i][j] << 120 - 8 * (4 * i + j)
"]
		25 -> 23 [label=""]
		23 -> 25 [label="range(4)"]
		23 -> 22 [label=""]
		22 -> 23 [label="range(4)"]
		24 [label="return text
"]
		22 -> 24 [label=""]
		21 -> 22 [label=""]
	}
	subgraph cluster__init__ {
		graph [label=__init__]
		30 [label="change_key(master_key)
"]
		"30_calls" [label=change_key shape=box]
		30 -> "30_calls" [label=calls style=dashed]
	}
	subgraph clusterchange_key {
		graph [label=change_key]
		33 [label="round_keys = text2matrix(master_key)
"]
		"33_calls" [label=text2matrix shape=box]
		33 -> "33_calls" [label=calls style=dashed]
		34 [label="for i in range(4, 4 * 11):
"]
		35 [label="round_keys.append([])
if i % 4 == 0:
"]
		"35_calls" [label="round_keys.append" shape=box]
		35 -> "35_calls" [label=calls style=dashed]
		37 [label="byte = round_keys[i - 4][0] ^ Sbox[round_keys[i - 1][1]] ^ Rcon[int(i / 4)]
round_keys[i].append(byte)
"]
		"37_calls" [label="int
round_keys.append" shape=box]
		37 -> "37_calls" [label=calls style=dashed]
		42 [label="for j in range(1, 4):
"]
		43 [label="byte = round_keys[i - 4][j] ^ Sbox[round_keys[i - 1][(j + 1) % 4]]
round_keys[i].append(byte)
"]
		"43_calls" [label="round_keys.append" shape=box]
		43 -> "43_calls" [label=calls style=dashed]
		43 -> 42 [label=""]
		42 -> 43 [label="range(1, 4)"]
		42 -> 34 [label=""]
		37 -> 42 [label=""]
		35 -> 37 [label="i % 4 == 0"]
		39 [label="for j in range(4):
"]
		40 [label="byte = round_keys[i - 4][j] ^ round_keys[i - 1][j]
round_keys[i].append(byte)
"]
		"40_calls" [label="round_keys.append" shape=box]
		40 -> "40_calls" [label=calls style=dashed]
		40 -> 39 [label=""]
		39 -> 40 [label="range(4)"]
		39 -> 34 [label=""]
		35 -> 39 [label="(i % 4 != 0)"]
		34 -> 35 [label="range(4, 4 * 11)"]
		33 -> 34 [label=""]
	}
	subgraph clusterencrypt {
		graph [label=encrypt]
		47 [label="plain_state = text2matrix(plaintext)
__add_round_key(plain_state, round_keys[:4])
"]
		"47_calls" [label="text2matrix
__add_round_key" shape=box]
		47 -> "47_calls" [label=calls style=dashed]
		48 [label="for i in range(1, 10):
"]
		49 [label="__round_encrypt(plain_state, round_keys[4 * i:4 * (i + 1)])
"]
		"49_calls" [label=__round_encrypt shape=box]
		49 -> "49_calls" [label=calls style=dashed]
		49 -> 48 [label=""]
		48 -> 49 [label="range(1, 10)"]
		50 [label="__sub_bytes(plain_state)
__shift_rows(plain_state)
__add_round_key(plain_state, round_keys[40:])
return matrix2text(plain_state)
"]
		"50_calls" [label="__sub_bytes
__shift_rows
__add_round_key" shape=box]
		50 -> "50_calls" [label=calls style=dashed]
		48 -> 50 [label=""]
		47 -> 48 [label=""]
	}
	subgraph clusterdecrypt {
		graph [label=decrypt]
		54 [label="cipher_state = text2matrix(ciphertext)
__add_round_key(cipher_state, round_keys[40:])
__inv_shift_rows(cipher_state)
__inv_sub_bytes(cipher_state)
"]
		"54_calls" [label="text2matrix
__add_round_key
__inv_shift_rows
__inv_sub_bytes" shape=box]
		54 -> "54_calls" [label=calls style=dashed]
		55 [label="for i in range(9, 0, -1):
"]
		56 [label="__round_decrypt(cipher_state, round_keys[4 * i:4 * (i + 1)])
"]
		"56_calls" [label=__round_decrypt shape=box]
		56 -> "56_calls" [label=calls style=dashed]
		56 -> 55 [label=""]
		55 -> 56 [label="range(9, 0, -1)"]
		57 [label="__add_round_key(cipher_state, round_keys[:4])
return matrix2text(cipher_state)
"]
		"57_calls" [label=__add_round_key shape=box]
		57 -> "57_calls" [label=calls style=dashed]
		55 -> 57 [label=""]
		54 -> 55 [label=""]
	}
	subgraph cluster__add_round_key {
		graph [label=__add_round_key]
		61 [label="for i in range(4):
"]
		62 [label="for j in range(4):
"]
		64 [label="s[i][j] ^= k[i][j]
"]
		64 -> 62 [label=""]
		62 -> 64 [label="range(4)"]
		62 -> 61 [label=""]
		61 -> 62 [label="range(4)"]
	}
	subgraph cluster__round_encrypt {
		graph [label=__round_encrypt]
		68 [label="__sub_bytes(state_matrix)
__shift_rows(state_matrix)
__mix_columns(state_matrix)
__add_round_key(state_matrix, key_matrix)
"]
		"68_calls" [label="__sub_bytes
__shift_rows
__mix_columns
__add_round_key" shape=box]
		68 -> "68_calls" [label=calls style=dashed]
	}
	subgraph cluster__round_decrypt {
		graph [label=__round_decrypt]
		71 [label="__add_round_key(state_matrix, key_matrix)
__inv_mix_columns(state_matrix)
__inv_shift_rows(state_matrix)
__inv_sub_bytes(state_matrix)
"]
		"71_calls" [label="__add_round_key
__inv_mix_columns
__inv_shift_rows
__inv_sub_bytes" shape=box]
		71 -> "71_calls" [label=calls style=dashed]
	}
	subgraph cluster__sub_bytes {
		graph [label=__sub_bytes]
		74 [label="for i in range(4):
"]
		75 [label="for j in range(4):
"]
		77 [label="s[i][j] = Sbox[s[i][j]]
"]
		77 -> 75 [label=""]
		75 -> 77 [label="range(4)"]
		75 -> 74 [label=""]
		74 -> 75 [label="range(4)"]
	}
	subgraph cluster__inv_sub_bytes {
		graph [label=__inv_sub_bytes]
		81 [label="for i in range(4):
"]
		82 [label="for j in range(4):
"]
		84 [label="s[i][j] = InvSbox[s[i][j]]
"]
		84 -> 82 [label=""]
		82 -> 84 [label="range(4)"]
		82 -> 81 [label=""]
		81 -> 82 [label="range(4)"]
	}
	subgraph cluster__shift_rows {
		graph [label=__shift_rows]
		88 [label="s[0][1] = s[1][1]
s[1][1] = s[2][1]
s[2][1] = s[3][1]
s[3][1] = s[0][1]
s[0][2] = s[2][2]
s[1][2] = s[3][2]
s[2][2] = s[0][2]
s[3][2] = s[1][2]
s[0][3] = s[3][3]
s[1][3] = s[0][3]
s[2][3] = s[1][3]
s[3][3] = s[2][3]
"]
	}
	subgraph cluster__inv_shift_rows {
		graph [label=__inv_shift_rows]
		91 [label="s[0][1] = s[3][1]
s[1][1] = s[0][1]
s[2][1] = s[1][1]
s[3][1] = s[2][1]
s[0][2] = s[2][2]
s[1][2] = s[3][2]
s[2][2] = s[0][2]
s[3][2] = s[1][2]
s[0][3] = s[1][3]
s[1][3] = s[2][3]
s[2][3] = s[3][3]
s[3][3] = s[0][3]
"]
	}
	subgraph cluster__mix_single_column {
		graph [label=__mix_single_column]
		94 [label="t = a[0] ^ a[1] ^ a[2] ^ a[3]
u = a[0]
a[0] ^= t ^ xtime(a[0] ^ a[1])
a[1] ^= t ^ xtime(a[1] ^ a[2])
a[2] ^= t ^ xtime(a[2] ^ a[3])
a[3] ^= t ^ xtime(a[3] ^ u)
"]
		"94_calls" [label="xtime
xtime
xtime
xtime" shape=box]
		94 -> "94_calls" [label=calls style=dashed]
	}
	subgraph cluster__mix_columns {
		graph [label=__mix_columns]
		97 [label="for i in range(4):
"]
		98 [label="__mix_single_column(s[i])
"]
		"98_calls" [label=__mix_single_column shape=box]
		98 -> "98_calls" [label=calls style=dashed]
		98 -> 97 [label=""]
		97 -> 98 [label="range(4)"]
	}
	subgraph cluster__inv_mix_columns {
		graph [label=__inv_mix_columns]
		102 [label="for i in range(4):
"]
		103 [label="u = xtime(xtime(s[i][0] ^ s[i][2]))
v = xtime(xtime(s[i][1] ^ s[i][3]))
s[i][0] ^= u
s[i][1] ^= v
s[i][2] ^= u
s[i][3] ^= v
"]
		"103_calls" [label="xtime
xtime" shape=box]
		103 -> "103_calls" [label=calls style=dashed]
		103 -> 102 [label=""]
		102 -> 103 [label="range(4)"]
		104 [label="__mix_columns(s)
"]
		"104_calls" [label=__mix_columns shape=box]
		104 -> "104_calls" [label=calls style=dashed]
		102 -> 104 [label=""]
	}
}
