...
class NameOnlyInstrumentor(ast.NodeTransformer):
    ...
    def clean_up_scope(self, cur_scope):
        for var in self.var_scopes:
            if len(self.var_scopes[var]) > 0 and self.var_scopes[var][-1] == cur_scope:
                self.var_scopes[var].pop()
        self.scope.pop()
    ...
    def visit_Call(self, node):
        ...
        if node.keywords:
            node.keywords = self.visit_Stmts(node.keywords)
            ...
            if type(node.value.func) == ast.Name:
                if "file" in node.value.func.id and "read" in node.value.func.id:
                    ...
                    if type(node.targets[0]) == ast.Name:
                        ...
                        raise Exception(
                            "Found file read but not name or tuple as target. node: {node}"
                        )
                    if node.value.args:
                        node.value.args = self.visit_Stmts(node.value.args)
                    return node
    ...
    def visit_FunctionDef(self, node):
        ...
        for arg in node.args.args:
            ...
            stmts.append(
                ast.Assign(targets=[name], value=ast.Name(id=arg, ctx=ast.Load()))
            )
    ...
    def visit_arguments(self, node: arguments) -> Any:
        if node.posonlyargs:
            node.posonlyargs = self.visit_Stmts(node.posonlyargs)
        if node.args:
            node.args = self.visit_Stmts(node.args)
        if node.kwonlyargs:
            node.kwonlyargs = self.visit_Stmts(node.kwonlyargs)
        if node.defaults:
            node.defaults = self.visit_Stmts(node.defaults)
        if node.vararg:
            node.vararg = self.visit(node.vararg)
        if node.kwarg:
            node.kwarg = self.visit(node.kwarg)
        if node.kw_defaults:
            node.kw_defaults = self.visit_Stmts(node.kw_defaults)
    ...
    def visit_arg(self, node):
        ...
        if node.arg not in self.dont_change_names:
            node.arg = self.get_name(node.arg, str(self.var_scopes[node.arg][-1]))
        return node
    ...
    def visit_Name(self, node):
        ...
        if type(node.ctx) == ast.Store:
            if node.id not in self.var_scopes:
                ...
                return ast.Name(
                    id=self.get_name(node.id, str(self.scope[-1])), ctx=node.ctx
                )
            else:
                while (
                    len(self.var_scopes[node.id]) > 0
                    and self.var_scopes[node.id][-1] not in self.valid_scopes
                ):
                    self.var_scopes[node.id].pop()
                if len(self.var_scopes[node.id]) > 0:
                    return ast.Name(
                        id=self.get_name(node.id, str(self.var_scopes[node.id][-1])),
                        ctx=node.ctx,
                    )
                else:
                    self.var_scopes[node.id] = deque([self.scope[-1]])
...
class NameScopeInstrumentor(ast.NodeTransformer):
    ...
    def visit_AugAssign(self, node: AugAssign) -> Any:
        if node.lineno not in lineno_to_node:
            return node
...
class ProgramInstrumentor(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        ...
        if type(node.targets[0]) == ast.Name:
            ...
            new_line = ast.Subscript(
                node.targets[0].value, node.targets[0].slice, ctx=ast.Load()
            )
    ...
    def visit_Call(self, node):
        # report("visiting function call",node)
        if type(node.func) == ast.Name and node.func.id == "print":
            return node
    ...
    def visit_Subscript(self, node: Subscript) -> Any:
        if type(node.ctx) == ast.Store:
            ...
            retval = ast.Call(
                ast.Name("instrument_read_sub", ast.Load()),
                args=[
                    node.value,
                    ast.Constant(t),
                    node.slice,
