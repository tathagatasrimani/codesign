...
def _load_benchmark_module(name, filename):
    """
    Utility to load a module from the local mvs-transistor-models/benchmarks
    directory whose name contains a hyphen (not valid as a Python package).
    """
    base_dir = os.path.dirname(__file__)
    benchmarks_dir = os.path.join(base_dir, "mvs-transistor-models", "benchmarks")
    module_path = os.path.join(benchmarks_dir, filename)
...
generate_netlists = _load_benchmark_module("generate_netlists", "generate_netlists.py")
analyze_fo4 = _load_benchmark_module("analyze_fo4", "analyze_fo4.py")
analyze_ring = _load_benchmark_module("analyze_ring", "analyze_ring.py")
analyze_transistor = _load_benchmark_module("analyze_transistor", "analyze_transistor.py")
...
class MVS1SpiceModel(TechModel):
    ...
    def set_output_folder(self, current_config):
        import hashlib
        # Use hash to avoid "file name too long" errors
        config_str = "_".join([f"{k}={v:.6g}" for k, v in sorted(current_config.items())])
    ...
    def init_transistor_equations(self):
        ...
        # Get VOL (low output voltage) from FO4: vout2 when vin=0
        vout2 = fo4_results['vout2']
        VOL = vout2[0]
        log_info(f"VOL: {VOL}")

        # Ioff: drain current at Vgs=0, Vds=VOL
        self.I_sub = analyze_transistor.get_id_at_bias(single_transistor_results, vgs_target=VOL, vds_target=self.V_dd)
        log_info(f"I_sub: {self.I_sub}")
        # Ieff: average of Ihigh and Ilow
        # Ihigh: Vgs=Vdd, Vds=Vdd/2
        # Ilow: Vgs=Vdd/2, Vds=Vdd
        Ihigh = analyze_transistor.get_id_at_bias(single_transistor_results, vgs_target=self.V_dd, vds_target=self.V_dd/2)
        Ilow = analyze_transistor.get_id_at_bias(single_transistor_results, vgs_target=self.V_dd/2, vds_target=self.V_dd)
        self.Ieff = 0.5 * (Ihigh + Ilow)
        ...
        #ring_results = analyze_ring.process_ring_simulation(os.path.join(self.output_folder, "ring_oscillator.scs"), num_ro_stages, plot=True, plot_file=os.path.join(self.output_folder, "ring_oscillator.png"))
        #log_info(f"Single transistor results: {single_transistor_results}")
        ...
        log_info(f"I_tunnel: {self.I_tunnel}")
        ...
        self.R_avg_inv = self.V_dd/(self.Ieff)

        #self.ring_oscillator_delay = ring_results['t_stage_delay_ps'] * 1e-3  # ns
    ...
    def config_sweep_output_db(self):
        ...
        self.sweep_output_db["ring_oscillator_delay"] = self.ring_oscillator_delay
