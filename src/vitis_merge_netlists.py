...
def parse_module(root_dir, current_module):
    ...
    for submodule_name, submodule_netlist in submodule_netlists.items():
        if submodule_netlist is not None:
            ...
            full_netlist = merge_netlists(root_dir, full_netlist, current_module, submodule_netlist, submodule_name)
            debug_print(f"Merged netlist for submodule {submodule_name} into full netlist.")

    ## write out the full netlist for the current module for debugging purposes
...
def merge_netlists(root_dir, full_netlist, current_module, submodule_netlist, submodule_name): 
    ...
    """
    debug_print(f"Merging netlist for submodule {submodule_name} into full netlist.")
    ...
    for n, d in full_netlist.nodes(data=True):
        ...
        if isinstance(bind_node, dict):
            debug_print(f"module instance mapping: {module_instance_mapping}")
            debug_print(f"submodule name: {submodule_name}")
            debug_print(f"module name: {current_module}")
            debug_print(f"node data: {d}")
            debug_print(f"current node name full: {d.get('name')}")
            ...
            if curr_node_full_name is None:
                ...
                exit(1)
                continue
            curr_node_submodule_name = '_'.join(curr_node_full_name.split('_')[1:-1])
