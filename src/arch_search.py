...
def generate_new_aladdin_arch(
    cfg: CFG, hw: HardwareModel, cfg_node_to_hw_map, data_path, id_to_node
):
    """
    TODO: UPDATE THIS ARCH SEARCH TO USE THE NEW MAPPING FOR CFG_NODE_TO_HW_MAP
            ...
            for elem in hw_need:
                if hardwareModel.num_nodes_with_func(hw.netlist, elem) < hw_need[elem]:
                    ...
                    for i in range(n, hw_need[elem]):
                        hw.netlist.add_node(
                            (elem + str(i)),
                            type="pe" if elem is not "Regs" else "memory",
                            function=elem,
                            in_use=False,
                            idx=i,
                        )
    ...
    for elem_name, elem_data in dict(hw.netlist.nodes.data()).items():
        if elem_data["function"] is not "Regs":
            for reg, reg_data in dict(regs).items():
                hw.netlist.add_edge((elem_name), reg)
