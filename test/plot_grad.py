...
def plot_diff(
    ...
    if csv_file_path:
        ...
        data = {
            "cache 90": [("Vdd", 92.51), ("Vth", 91.71), ("C_g_ideal", 86.23)],
            "cache 45": [
                ("Vdd", 70),
                ("Vth", 60),
                ("C_g_ideal", 50),
    ...
    plt.savefig(os.path.join(figs_dir, f"grad_{name}.png"), bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="base_cache",
        help="Path or Name to the configuration file; don't prepend src/cacti/ or append .cfg",
    )

    args = parser.parse_args()

    print(f"Plotting gradient validation for {args.config}")
    ...
    # INDIVIDUAL PLOTS
    access_time_file = os.path.join(
        current_directory, "results", f"{cfg_name}_access_time_grad_results.csv"
    )
    plot_diff(access_time_file, name=f"{cfg_name}_access_time_similarity")

    read_energy_file = os.path.join(
        current_directory, "results", f"{cfg_name}_read_dynamic_grad_results.csv"
    )
    ...
    leakage_power_file = os.path.join(
        current_directory, "results", f"{cfg_name}_read_leakage_grad_results.csv"
    )
    ...
    combined_csv_path = os.path.join(
        current_directory, f"results/{cfg_name}_combined.csv"
    )
    ...
    plot_diff(
        ...
        False,
        16,
        name=f"{cfg_name}_combined_gradient_similarity",
    )
