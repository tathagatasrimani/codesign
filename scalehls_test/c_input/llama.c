...
/* RMS Normalization: x / sqrt(mean(x^2) + eps) * weight */
void rms_norm_decode(
  DATA_TYPE POLYBENCH_2D(x_out, 1, EMBED_DIM, seq_len, embed_dim),
  DATA_TYPE POLYBENCH_2D(x_in, 1, EMBED_DIM, seq_len, embed_dim),
  DATA_TYPE POLYBENCH_1D(weight, EMBED_DIM, embed_dim),
  int seq_len,
  int embed_dim,
  DATA_TYPE norm_eps)
...
/* SiLU activation: x * sigmoid(x) = x / (1 + exp(-x)) */
void silu_activation_decode(
  DATA_TYPE POLYBENCH_2D(x_out, 1, FF_DIM, seq_len, ff_dim),
  DATA_TYPE POLYBENCH_2D(x_in, 1, FF_DIM, seq_len, ff_dim),
  int seq_len,
  int ff_dim)
{
int i, j;
...
/* Standard linear layer: out = x @ weight^T + bias */
void linear_layer_decode(
  DATA_TYPE POLYBENCH_2D(out, 1, EMBED_DIM, seq_len, embed_dim_out),
  DATA_TYPE POLYBENCH_2D(x_in, 1, EMBED_DIM, seq_len, embed_dim_in),
  DATA_TYPE POLYBENCH_2D(weight, EMBED_DIM, EMBED_DIM, embed_dim_out, embed_dim_in),
  DATA_TYPE POLYBENCH_1D(bias, EMBED_DIM, embed_dim_out),
  int seq_len,
  int embed_dim_in,
...
/* Linear layer for FF_DIM output dimensions */
void linear_layer_ff_out_decode(
  DATA_TYPE POLYBENCH_2D(out, 1, FF_DIM, seq_len, embed_dim_out),
  DATA_TYPE POLYBENCH_2D(x_in, 1, EMBED_DIM, seq_len, embed_dim_in),
  DATA_TYPE POLYBENCH_2D(weight, FF_DIM, EMBED_DIM, embed_dim_out, embed_dim_in),
  DATA_TYPE POLYBENCH_1D(bias, FF_DIM, embed_dim_out),
  int seq_len,
...
/* Linear layer for FF_DIM input dimensions */
void linear_layer_ff_in_decode(
  DATA_TYPE POLYBENCH_2D(out, 1, EMBED_DIM, seq_len, embed_dim_out),
  DATA_TYPE POLYBENCH_2D(x_in, 1, FF_DIM, seq_len, embed_dim_in),
  DATA_TYPE POLYBENCH_2D(weight, EMBED_DIM, FF_DIM, embed_dim_out, embed_dim_in),
  DATA_TYPE POLYBENCH_1D(bias, EMBED_DIM, embed_dim_out),
  int seq_len,
  int embed_dim_in,
...
/* Feed-forward network: w2(silu(w1(x)) * w3(x)) */
void feed_forward_decode(
  DATA_TYPE POLYBENCH_2D(out, 1, EMBED_DIM, seq_len, embed_dim),
  DATA_TYPE POLYBENCH_2D(x_in, 1, EMBED_DIM, seq_len, embed_dim),
  DATA_TYPE POLYBENCH_2D(w1_weight, FF_DIM, EMBED_DIM, ff_dim, embed_dim),
  DATA_TYPE POLYBENCH_2D(w3_weight, FF_DIM, EMBED_DIM, ff_dim, embed_dim),
  DATA_TYPE POLYBENCH_2D(w2_weight, EMBED_DIM, FF_DIM, embed_dim, ff_dim),
...
{
int i, j;
DATA_TYPE POLYBENCH_2D(w1_out, 1, FF_DIM, seq_len, ff_dim);
DATA_TYPE POLYBENCH_2D(w3_out, 1, FF_DIM, seq_len, ff_dim);
DATA_TYPE POLYBENCH_2D(silu_out, 1, FF_DIM, seq_len, ff_dim);
DATA_TYPE POLYBENCH_2D(mult_out, 1, FF_DIM, seq_len, ff_dim);
...
linear_layer_ff_out_decode(w1_out, x_in, w1_weight, w1_bias, seq_len, embed_dim, ff_dim);
...
linear_layer_ff_out_decode(w3_out, x_in, w3_weight, w3_bias, seq_len, embed_dim, ff_dim);

/* SiLU activation on w1_out */
silu_activation_decode(silu_out, w1_out, seq_len, ff_dim);
...
linear_layer_ff_in_decode(out, mult_out, w2_weight, w2_bias, seq_len, ff_dim, embed_dim);
}

/* Precompute RoPE frequencies: freqs_cis[i] = (cos, sin) for position i */
/* Standard RoPE: for pair i (dimensions 2i, 2i+1), freq = 1 / (theta^(2i/head_dim)) */
void precompute_freqs_cis_decode(
...
/* Apply rotary position embedding to queries and keys */
void apply_rotary_emb_decode(
  DATA_TYPE POLYBENCH_4D(q_out, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, heads, seq_len, head_dim),
  DATA_TYPE POLYBENCH_4D(k_out, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, heads, seq_len, head_dim),
  DATA_TYPE POLYBENCH_4D(q_in, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, heads, seq_len, head_dim),
  DATA_TYPE POLYBENCH_4D(k_in, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, heads, seq_len, head_dim),
  DATA_TYPE POLYBENCH_2D(freqs_cis, SEQ_LEN, HEAD_DIM, seq_len, head_dim),
  int batch_size,
  int heads,
  int seq_len,
...
for (b = 0; b < BATCH_SIZE; b++) {
  for (h = 0; h < HEADS; h++) {
    for (i = 0; i < 1; i++) {
      for (j = 0; j < HEAD_DIM; j += 2) {
        DATA_TYPE cos_val = freqs_cis[start_pos + i][j];
        DATA_TYPE sin_val = freqs_cis[start_pos + i][j + 1];
        DATA_TYPE q0 = q_in[b][h][i][j];
        DATA_TYPE q1 = q_in[b][h][i][j + 1];
        DATA_TYPE k0 = k_in[b][h][i][j];
        DATA_TYPE k1 = k_in[b][h][i][j + 1];
...
/* Reshape 2D tensor (seq_len, embed_dim) to 4D tensor (batch, heads, seq_len, head_dim) */
void reshape_to_heads_4d_decode(
  DATA_TYPE POLYBENCH_4D(qkv_4d, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, heads, seq_len, head_dim),
  DATA_TYPE POLYBENCH_2D(qkv_2d, 1, EMBED_DIM, seq_len, embed_dim),
  int batch_size,
  int seq_len,
  int embed_dim,
  int heads,
...
/* Reshape 4D tensor (batch, heads, seq_len, head_dim) back to 2D tensor (seq_len, embed_dim) */
void reshape_from_heads_4d_decode(
  DATA_TYPE POLYBENCH_2D(out_2d, 1, EMBED_DIM, seq_len, embed_dim),
  DATA_TYPE POLYBENCH_4D(out_4d, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, heads, seq_len, head_dim),
  int batch_size,
  int seq_len,
  int embed_dim,
  int heads,
...
/* Expand K/V from kv_heads to query_heads by repeating */
void expand_kv_heads_4d_decode(
  DATA_TYPE POLYBENCH_4D(kv_expanded, BATCH_SIZE, HEADS, SEQ_LEN, HEAD_DIM, batch_size, query_heads, seq_len, head_dim),
  DATA_TYPE POLYBENCH_4D(kv_original, BATCH_SIZE, HEADS, SEQ_LEN, HEAD_DIM, batch_size, kv_heads, seq_len, head_dim),
  int batch_size,
  int seq_len,
...
void scaled_dot_product_attention_decode(
  DATA_TYPE POLYBENCH_4D(attn_out, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, heads, seq_len, head_dim),
  DATA_TYPE POLYBENCH_4D(query, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, heads, seq_len_q, head_dim),
  DATA_TYPE POLYBENCH_4D(key, BATCH_SIZE, HEADS, SEQ_LEN, HEAD_DIM, batch_size, heads, seq_len_kv, head_dim),
  DATA_TYPE POLYBENCH_4D(value, BATCH_SIZE, HEADS, SEQ_LEN, HEAD_DIM, batch_size, heads, seq_len_kv, head_dim),
  DATA_TYPE POLYBENCH_2D(attn_mask, SEQ_LEN, SEQ_LEN, seq_len_q, seq_len_kv),
  int batch_size,
...
int b, h, i, j, k;
DATA_TYPE scale = SCALAR_VAL(1.0) / SQRT_FUN((DATA_TYPE)head_dim);
DATA_TYPE POLYBENCH_3D(similarity, HEADS, 1, SEQ_LEN, heads, seq_len_q, seq_len_kv);
DATA_TYPE POLYBENCH_3D(attention, HEADS, 1, SEQ_LEN, heads, seq_len_q, seq_len_kv);
...
{
int i, j, h, d;
DATA_TYPE POLYBENCH_2D(q, 1, EMBED_DIM, seq_len, embed_dim);
DATA_TYPE POLYBENCH_2D(k, 1, EMBED_DIM, seq_len, kv_embed_dim);
DATA_TYPE POLYBENCH_2D(v, 1, EMBED_DIM, seq_len, kv_embed_dim);
...
linear_layer_decode(q, x_in, q_weight, q_bias, seq_len, embed_dim, embed_dim);
linear_layer_decode(k, x_in, k_weight, k_bias, seq_len, embed_dim, kv_embed_dim);
linear_layer_decode(v, x_in, v_weight, v_bias, seq_len, embed_dim, kv_embed_dim);

/* Reshape to 4D: (batch, heads, seq_len, head_dim) */
DATA_TYPE POLYBENCH_4D(q_4d, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, query_heads, seq_len, head_dim);
DATA_TYPE POLYBENCH_4D(k_4d, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, kv_heads, seq_len, head_dim);
DATA_TYPE POLYBENCH_4D(v_4d, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, kv_heads, seq_len, head_dim);
...
/* Apply RoPE */
apply_rotary_emb_decode(q_4d, k_4d, q_4d, k_4d, freqs_cis, batch_size, query_heads, seq_len, head_dim, start_pos);
...
DATA_TYPE POLYBENCH_4D(k_expanded, BATCH_SIZE, HEADS, SEQ_LEN, HEAD_DIM, batch_size, query_heads, total_seq_len, head_dim);
DATA_TYPE POLYBENCH_4D(v_expanded, BATCH_SIZE, HEADS, SEQ_LEN, HEAD_DIM, batch_size, query_heads, total_seq_len, head_dim);
expand_kv_heads_4d_decode(k_expanded, k_cached, batch_size, total_seq_len, head_dim, query_heads, kv_heads);
expand_kv_heads_4d_decode(v_expanded, v_cached, batch_size, total_seq_len, head_dim, query_heads, kv_heads);

/* Compute attention */
DATA_TYPE POLYBENCH_4D(attn_out_4d, BATCH_SIZE, HEADS, 1, HEAD_DIM, batch_size, query_heads, seq_len, head_dim);
int use_attn_mask = use_mask;
scaled_dot_product_attention_decode(attn_out_4d, q_4d, k_expanded, v_expanded, attn_mask,
                              batch_size, query_heads, seq_len, total_seq_len, head_dim, use_attn_mask);
...
/* Output projection */
DATA_TYPE POLYBENCH_2D(attn_out_proj, 1, EMBED_DIM, seq_len, embed_dim);
DATA_TYPE POLYBENCH_1D(o_bias, EMBED_DIM, embed_dim);
for (i = 0; i < EMBED_DIM; i++) {
  o_bias[i] = SCALAR_VAL(0.0);
}
linear_layer_decode(attn_out_proj, out, o_weight, o_bias, seq_len, embed_dim, embed_dim);
...
{
int i, j;
DATA_TYPE POLYBENCH_2D(attn_norm_out, 1, EMBED_DIM, seq_len, embed_dim);
DATA_TYPE POLYBENCH_2D(attn_out, 1, EMBED_DIM, seq_len, embed_dim);
DATA_TYPE POLYBENCH_2D(attn_residual, 1, EMBED_DIM, seq_len, embed_dim);
DATA_TYPE POLYBENCH_2D(ffn_norm_out, 1, EMBED_DIM, seq_len, embed_dim);
DATA_TYPE POLYBENCH_2D(ffn_out, 1, EMBED_DIM, seq_len, embed_dim);

/* Attention with pre-norm and residual */
rms_norm_decode(attn_norm_out, x_in, attn_norm_weight, seq_len, embed_dim, NORM_EPS);
gqa_attention_decode(attn_out, attn_norm_out, q_weight, k_weight, v_weight, o_weight,
              cache_k, cache_v, freqs_cis, attn_mask, use_mask,
              batch_size, seq_len, start_pos, embed_dim, kv_embed_dim,
              query_heads, kv_heads, head_dim);
...
/* Feed-forward with pre-norm and residual */
rms_norm_decode(ffn_norm_out, attn_residual, ffn_norm_weight, seq_len, embed_dim, NORM_EPS);
feed_forward_decode(ffn_out, ffn_norm_out, w1_weight, w3_weight, w2_weight,
             seq_len, embed_dim, ff_dim);
...
/* Final RMSNorm */
rms_norm_decode(output_norm, x_transformed, output_norm_weight, 1, embed_dim, NORM_EPS);
...
void llama(
  ...
  //precompute_freqs_cis(freqs_cis, max_seq_len, head_dim, ROPE_THETA);
  ...
  DATA_TYPE POLYBENCH_2D(next_layer_input, 1, VOCAB_SIZE, 1, vocab_size);
  for (int i = 0; i < VOCAB_SIZE; i++) {
    next_layer_input[0][i] = out[SEQ_LEN - 1][i];
  }
  DATA_TYPE POLYBENCH_1D(next_token, 1, 1);
  ...
  for (unsigned i = 0; i < NUM_DECODE_ITERS; i++) {
    llama_decode(next_layer_input, next_token, emb_weight,
                 q_weights, k_weights, v_weights, o_weights,
                 w1_weights, w3_weights, w2_weights,
                 attn_norm_weights, ffn_norm_weights, output_weight,
                 cache_k, cache_v, freqs_cis, attn_mask,
