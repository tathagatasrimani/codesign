...
class OpenRoadRun:
    ...
    def estimated_place_n_route(
        ...
        route_guide_path = self._resolve_route_guide_path()
    ...
    def _resolve_route_guide_path(self) -> str:
        """
        Resolve the route-guide file generated by codesign_flow.tcl.

        Preferred path is the canonical file used by legacy parsing.
        If retries were used and only attempt-specific files exist,
        pick the latest attempt.
        """
        canonical = os.path.join(self.directory, "results", "codesign_codesign-tcl.route_guide")
        if os.path.exists(canonical):
            return canonical

        candidates = sorted(glob.glob(canonical + ".a*"))
        if candidates:
            chosen = candidates[-1]
            logger.warning(
                f"Canonical route guide not found. Falling back to retry output: {chosen}"
            )
            return chosen

        raise FileNotFoundError(
            f"Route guide not found. Expected {canonical} or retry files {canonical}.a*"
        )
